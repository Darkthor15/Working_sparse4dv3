...
class RectifyImageNode(Node):
    def __init__(self):
        ...
        self.image_sub = self.create_subscription(
            Image, '/CAM_FRONT_LEFT/image_8bit_color', self.image_callback, 10)
        self.camera_info_sub = self.create_subscription(
            CameraInfo, '/CAM_FRONT_LEFT/camera_info', self.camera_info_callback, 10)
    ...
    def image_callback(self, msg):
        if self.map1 is None or self.map2 is None:
            return  # Wait until calibration info is received and maps are computed
