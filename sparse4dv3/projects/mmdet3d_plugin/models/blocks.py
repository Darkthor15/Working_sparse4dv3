...
from ..ops import group_weights_generation_function
from ..ops import keypoints_projection_function as project_keypoints
...
def get_learnable_params(layers_list):
    weights = []
    biases = []
    scales = []
    for layers in layers_list:
        for module in layers.modules():
            if isinstance(module, Linear) or isinstance(module, nn.LayerNorm):
...
def group_weights_generation(instance_feature, anchor_embed, projection_mat, camera_encoder, weights_fc, num_groups):
    weights, biases, scales = get_learnable_params([ camera_encoder, weights_fc ])
    return group_weights_generation_function(instance_feature, anchor_embed, projection_mat,
        weights, biases, weights_fc.out_features, num_groups)
...
class DeformableFeatureAggregation(BaseModule):
    def __init__(
        ...
        self.use_group_weights_kernel = True
    ...
    def forward(
        ...
        **kwargs: dict,
        ...
        with record_function("WeightsGeneration"):
            weights = self._get_weights(instance_feature, anchor_embed, metas)

        if self.use_deformable_func:
            with record_function("ProjectPoints"):
                points_2d = self.project_points_opt(key_points, metas["projection_mat"], metas.get("image_wh"))
            weights = (
    ...
    def _get_weights(self, instance_feature, anchor_embed, metas=None):
        bs, num_anchor = instance_feature.shape[:2]
        if self.use_group_weights_kernel and not self.training and self.camera_encoder is not None:
            projection_mat = metas["projection_mat"]
            weights = group_weights_generation(instance_feature, anchor_embed, projection_mat, self.camera_encoder, self.weights_fc, self.num_groups)
            weights = weights.reshape(bs, num_anchor, self.num_cams, self.num_levels, self.num_pts, self.num_groups)
            return weights
    ...
    @staticmethod
    def project_points_opt(key_points, projection_mat, image_wh=None):
        return project_keypoints(key_points, projection_mat, image_wh)
