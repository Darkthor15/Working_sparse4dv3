...
from ..ops import graph_model_function
...
class Sparse4DHead(BaseModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.temp_anchor_encoder = build(anchor_encoder, POSITIONAL_ENCODING)
    ...
    def get_learnable_params(self, index):
        weights = []
        biases = []

        # FC before
        weights.append(self.fc_before.weight)
        # biases.append(self.fc_before.bias)

        # MHA
        mha = self.layers[index]
        weights.append(mha.attn.in_proj_weight)
        biases.append(mha.attn.in_proj_bias)
        weights.append(mha.attn.out_proj.weight)
        biases.append(mha.attn.out_proj.bias)

        # FC after
        weights.append(self.fc_after.weight)
        # biases.append(self.fc_after.bias)
    ...
    def graph_model(
        ...
        **kwargs,
        ...
        if self.decouple_attn:
            with record_function("GenerateGraphParams"):
                weights, biases = self.get_learnable_params(index)
            mha = self.layers[index]
            return graph_model_function(query, key, value, query_pos, key_pos, weights, biases, mha.num_heads, mha.embed_dims//mha.num_heads)
        ...
        with torch.backends.cuda.sdp_kernel(enable_flash=False, enable_math=True, enable_mem_efficient=True):
            output = self.fc_after(self.layers[index](query, key, value, query_pos=query_pos, key_pos=key_pos, **kwargs,))
            return output
    ...
    def forward(
        ...
        with record_function("SparseHeadForward"):
            ...
            with record_function("InstanceBankGet"):
                (
                    instance_feature,
                    anchor,
            ...
            with record_function("DenoiseForTraining"):
            ...
            with record_function("Image Encoder"):
                anchor_embed = self.anchor_encoder(anchor)
            if temp_anchor is not None:
                with record_function("Non-TempAnchorEncoder"):
            ...
            for i, op in enumerate(self.operation_order):
                if self.layers[i] is None:
                    ...
                    with record_function("TemporalGraphModel"):
                    ...
                    with record_function("GraphModel"):
                        instance_feature = self.graph_model(
                            i,
                            instance_feature,
                    ...
                    with record_function("AsymmetricFFN"):
                        instance_feature = self.layers[i](instance_feature)
                elif op == "deformable":
                    with record_function("DeformableFeatureAggregation"):
                    ...
                    with record_function("SparseBox3DRefinementModule"):
                    ...
                    if len(prediction) == self.num_single_frame_decoder:
                        with record_function("InstanceBankUpdate"):
                        ...
                        if (
                            ...
                            with record_function("SparseBox3DTargetSamplerUpdate"):
                                (
                                    instance_feature,
                                    anchor,
                                    temp_dn_reg_target,
                    ...
                    if i != len(self.operation_order) - 1:
                        with record_function("TempAnchorEncoder"):
                            anchor_embed = self.anchor_encoder(anchor)
                    if (
                        len(prediction) > self.num_single_frame_decoder
                        and temp_anchor_embed is not None
                    ):
                        with record_function("TempAnchorEmbed"):
            ...
            with record_function("SplitPreds"):
                if dn_metas is not None:
                    dn_classification = [
            ...
            with record_function("InstanceBankCache"):
                self.instance_bank.cache(
                    instance_feature, anchor, cls, metas, feature_maps
                )
            if not self.training:
                with record_function("GetInstanceIDs"):
