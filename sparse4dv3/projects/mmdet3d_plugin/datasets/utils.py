...
def draw_lidar_bbox3d_on_img(
    ...
    if isinstance(bboxes3d, torch.Tensor):
        bboxes_np = bboxes3d.detach().cpu().numpy()

    velocities = bboxes_np[:, 7:10]

    # for i in range(num_bbox):
    #     pts = imgfov_pts_2d[i]
    #     # Compute a simple 2D bounding rectangle from the projected corners.
    #     x_min = int(np.min(pts[:, 0]))
    #     y_max = int(np.max(pts[:, 1]))
    #     # Prepare the label text.
    #     label = f"normalised : {math.sqrt(velocities[i, 1]**2 + velocities[i, 0]**2 ):.2f}"# , vz:{velocities[i, 2]:.2f} vx:{velocities[i, 0]:.2f}, vy:{velocities[i, 1]:.2f} ,
    #     # Draw the text slightly below the bottom-left corner of the box.
    #     cv2.putText(
    #         img,
    #         label,
    #         (x_min, y_max + 15),
...
def draw_lidar_bbox3d_on_bev(
    bboxes_3d, bev_size, bev_range=115, color=(255, 0, 0), thickness=2):
